...
CREATE TABLE if not exists peers (
    ...
    speed_limit_mbps float,  -- MegaBYTES not bits
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_peers_virtual_ip ON peers(virtual_ip);

-- Таблица правил (привязаны к конкретному пиру)
CREATE TABLE if not exists  peers_rules (
    id                INTEGER PRIMARY KEY AUTOINCREMENT,
